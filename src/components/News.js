...
const News=(props)=>{
  ...
  document.title=`NewsMonk - ${props.category}`
    ...
    return (
      <>
        <h1 className="text-center" style={{marginTop: '85px',marginBottom:'25px'}}>Top Headlines</h1>
        {loading && <Spinner/>}
        <InfiniteScroll dataLength={articles.length} next={fetchData} hasMore={articles.length<totalResults} loader={<Spinner/>} >
